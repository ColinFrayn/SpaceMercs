...
namespace SpaceMercs {
  class WeaponType : ItemType {
    ...
    public WeaponType(XmlNode xml) : base(xml) {
      Range = double.Parse(xml.SelectSingleNode("Range").InnerText);
      Speed = double.Parse(xml.SelectSingleNode("Speed").InnerText);
      ...
      if (xml.SelectSingleNode("Type") != null) DType = (DamageType)Enum.Parse(typeof(DamageType), xml.SelectSingleNode("Type").InnerText);

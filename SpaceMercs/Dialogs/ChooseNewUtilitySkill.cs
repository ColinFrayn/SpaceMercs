...
namespace SpaceMercs.Dialogs {
    partial class ChooseNewUtilitySkill : Form {
        ...
        private void lbSkills_DoubleClick(object sender, EventArgs e) {
            if (lbSkills.SelectedIndex < 0) return;
            Soldier.UtilitySkill? choice = (Soldier.UtilitySkill)Enum.Parse(typeof(Soldier.UtilitySkill), lbSkills.SelectedItem?.ToString() ?? string.Empty);
            if (choice is not null) MessageBox.Show(this, Utils.UtilitySkillToDesc(choice.Value));
        }
    }
}

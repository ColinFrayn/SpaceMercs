...
namespace SpaceMercs.Dialogs {
  partial class TeamView : Form {
    ...
    public void ShowSelectedSoldierDetails() {
      ...
      lbIntellect.Text = s.Intellect.ToString();
      ttSoldier.SetToolTip(this.lbIntellect, s.BaseIntellect + " (base) + " + s.StatBonuses(StatType.Intelligence) + " (items)");
      ...
      lbHealthTotal.Text = s.MaxHealth.ToString();
      ...
      foreach (WeaponType wp in s.SkilledWeapons) {
        int lvl = s.GetSoldierSkillWithWeapon(wp);
    ...
    private void btDrop_Click(object sender, EventArgs e) {
      ...
      IItem it = SelectedItem();
      int iPrevIndex = -1;
      if (lbEquipped.SelectedIndex >= 0 && (it is IEquippable)) s.Unequip((IEquippable)it);
    ...
    private void tbSkills_SelectedIndexChanged(object sender, EventArgs e) {
      ...
      if (s == null || tbSkills.SelectedIndex == 0) {

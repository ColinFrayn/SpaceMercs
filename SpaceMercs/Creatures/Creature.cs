...
namespace SpaceMercs {
  class Creature : IEntity {
    ...
    public void Display(bool bLabel, bool bStatBars, bool bShowEffects, float fViewHeight) {
      ...
      GL.PushMatrix();
      GL.Translate(X + ((float)Type.Size / 2), Y + ((float)Type.Size / 2), Const.EntityLayer);
      ...
      if (bStatBars) {
        ...
        GL.Color3(1.0, 1.0, 1.0);
        GL.PushMatrix();
        GL.Translate(X + ((float)Type.Size / 2), Y + ((float)Type.Size * 0.25f), Const.EntityLayer);
        GL.Scale(Type.Scale, Type.Scale, 1.0);
    ...
    public void ApplyEffectToEntity(IEntity src, ItemEffect ie, VisualEffect.EffectFactory fact) {
      ...
      foreach (Effect eff in ie.Effects) {
        if (eff.Duration == 0) {
          ...
          double dmod = 1.0;
          if (ie.AssociatedSkill != Soldier.UtilitySkill.Unspent && src is Soldier) {
    ...
    public void AttackEntity(IEntity en, VisualEffect.EffectFactory effectFactory, Action<string> playSound) {
      ...
      if (EquippedWeapon != null && !EquippedWeapon.Type.IsMeleeWeapon) {
        ...
        effectFactory(VisualEffect.EffectType.Shot, X, Y, new Dictionary<string, object>() { { "FX", X + 0.5 }, { "TX", en.X + 0.5 }, { "FY", Y + 0.5 }, { "TY", en.Y + 0.5 }, { "Power", pow }, { "Colour", Color.FromArgb(255, 200, 200, 200) } });
      }

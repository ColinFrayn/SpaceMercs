...
namespace SpaceMercs {
  class ShipEngine : ShipEquipment {
    ...
    public ShipEngine(XmlNode xml) : base(xml, ShipEquipment.RoomSize.Engine) {
      Range = Double.Parse(xml.SelectSingleNode("Range").InnerText) * Const.LightYear;
      Speed = Double.Parse(xml.SelectSingleNode("Speed").InnerText) * Const.SpeedOfLight;
      Accel = Double.Parse(xml.SelectSingleNode("Accel").InnerText);
    }

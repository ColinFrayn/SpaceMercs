...
namespace SpaceMercs {
  class Ship {
    ...
    public void DrawSchematic(int iHover, int iBuildingTexture, bool bHoverHull) {
      double maxy = 0.0;
      Tuple<double, double> tp = null;
      if (ArmourType != null) tp = Textures.GetTexCoords(ArmourType);
            ...
            for (int y = r.YPos - 1; y <= r.YPos + r.Height; y++) {
              for (int x = r.XPos - 1; x <= r.XPos + r.Width; x++) {
                if (y == r.YPos - 1 || x == r.XPos - 1 || y == r.YPos + r.Height || x == r.XPos + r.Width) {
                  GL.TexCoord2(tp.Item1, tp.Item2 + Textures.BuildingTextureHeight);
                  GL.Vertex3(x, y, 0.0);
                  GL.TexCoord2(tp.Item1 + Textures.BuildingTextureWidth, tp.Item2 + Textures.BuildingTextureHeight);
                  GL.Vertex3(x + 1, y, 0.0);
                  GL.TexCoord2(tp.Item1 + Textures.BuildingTextureWidth, tp.Item2);
      ...
      if (Type.Fillers.Any()) {
        ...
        foreach (Point pt in Type.Fillers) {
          if (ArmourType != null) GL.TexCoord2(tp.Item1, tp.Item2 + Textures.BuildingTextureHeight);
          GL.Vertex3(pt.X, pt.Y, 0.0);
          if (ArmourType != null) GL.TexCoord2(tp.Item1 + Textures.BuildingTextureWidth, tp.Item2 + Textures.BuildingTextureHeight);
          GL.Vertex3(pt.X + 1, pt.Y, 0.0);
          if (ArmourType != null) GL.TexCoord2(tp.Item1 + Textures.BuildingTextureWidth, tp.Item2);
          GL.Vertex3(pt.X + 1, pt.Y + 1, 0.0);
          if (ArmourType != null) GL.TexCoord2(tp.Item1, tp.Item2);

  ...
  internal static class Square {
    public static GLShape BuildTextured(Alignment ali) {
      ...
      VertexPos2DTex[] vertices = new VertexPos2DTex[] {
        new VertexPos2DTex(new Vector2(tlcx,    tlcy-1f), new Vector2(0f, 0f)),
        new VertexPos2DTex(new Vector2(tlcx,    tlcy),    new Vector2(0f, 1f)),
        new VertexPos2DTex(new Vector2(tlcx+1f, tlcy),    new Vector2(1f, 1f)),
        new VertexPos2DTex(new Vector2(tlcx+1f, tlcy-1f), new Vector2(1f, 0f)),
      };
      int[] indices = new int[6] { 0, 1, 2, 0, 2, 3 };
      return new GLShape(vertices, indices);
    }
    public static GLShape BuildMultiColoured(Alignment ali) { // DEBUG REMOVEME
      float tlcx = 0f, tlcy = 0f;
      if (ali == Alignment.TopMiddle || ali == Alignment.CentreMiddle || ali == Alignment.BottomMiddle) tlcx = -0.5f;
      if (ali == Alignment.TopRight || ali == Alignment.CentreRight || ali == Alignment.BottomRight) tlcx = -1.0f;
      if (ali == Alignment.CentreLeft || ali == Alignment.CentreMiddle || ali == Alignment.CentreRight) tlcy = 0.5f;
      ...
      VertexPos2DCol[] vertices = new VertexPos2DCol[] {
        new VertexPos2DCol(new Vector2(tlcx,    tlcy-1f), new Color4(1f, 0f, 0f, 1f)),
        new VertexPos2DCol(new Vector2(tlcx,    tlcy),    new Color4(0f, 1f, 0f, 1f)),
        new VertexPos2DCol(new Vector2(tlcx+1f, tlcy),    new Color4(0f, 0f, 1f, 1f)),
        new VertexPos2DCol(new Vector2(tlcx+1f, tlcy-1f), new Color4(1f, 1f, 0f, 1f)),
      };
      int[] indices = new int[6] { 0, 1, 2, 0, 2, 3 };
      return new GLShape(vertices, indices);
    }

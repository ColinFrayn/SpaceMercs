...
namespace SpaceMercs {
  class Team {
    ...
    public Team(NewGame ng, Race rc) {
      CurrentPosition = rc.HomePlanet;
      Random rand = new Random();
      Soldier s = new Soldier(ng.PlayerName, rc, ng.Strength, ng.Agility, ng.Intellect, ng.Toughness, ng.Endurance, GenderType.Male, 1, rand.Next());
      s.PlayerTeam = this;
      _Soldiers.Add(s);
    ...
    public Team(XmlNode xml, Map map) {
      ...
      foreach (XmlNode xr in xmlr.ChildNodes) {
        ...
        int rel = Int32.Parse(xr.Attributes["Value"].Value);

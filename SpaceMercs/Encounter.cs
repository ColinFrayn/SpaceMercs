...
namespace SpaceMercs {
  static class Encounter {
    ...
    public static Mission CheckForInterception(AstronomicalObject aoFrom, AstronomicalObject aoTo, double dJourneyTime, Team PlayerTeam, double dFract) {
      ...
      double dDanger = Const.BaseEncounterFrequency + Const.DEBUG_ENCOUNTER_FREQ_MOD;
      ...
      double dIntercept = rand.NextDouble() * 200.0;
    ...
    private static Mission InactiveEncounter(Race rc, double dDanger, Random rand, int iDiff, Team PlayerTeam, ShipEngine minDrive) {
      ...
      if (rc == null) bLifeForms = (rand.NextDouble() > 0.5);
        ...
        if (bLifeForms) {
          ...
          if (bFriendly) {
            ...
            return Mission.CreateRepairMission(rc, dTime * Const.SecondsPerDay, dReward);
          }
        }
      }
      ...
      if (!bLifeForms) {
        ...
        return Mission.CreateSalvageMission(rc, iDiff, dTime * Const.SecondsPerDay);
      }
